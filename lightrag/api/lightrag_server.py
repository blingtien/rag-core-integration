...
from lightrag.llm.openai import openai_complete_if_cache, openai_complete
...
def create_app(args):
    ...
    # Initialize HuggingFace model and tokenizer if using HF embedding
    hf_tokenizer = None
    hf_model = None
    if args.embedding_binding == "hf":
        ...
        # Initialize HF model and tokenizer for local embedding
        ...
        logger.info(f"Loaded HF embedding model: {args.embedding_model}")
    ...
    def create_optimized_azure_openai_llm_func(
        ...
        async def optimized_azure_openai_model_complete(
            ...
            **kwargs,
            ...
            from lightrag.llm.azure_openai import azure_openai_complete_if_cache, azure_openai_complete
    ...
    def create_optimized_embedding_function(
        config_cache: LLMConfigCache, binding, model, host, api_key, dimensions, args, hf_tokenizer=None, hf_model=None
    ):
        """
    ...
    embedding_func = EmbeddingFunc(
        ...
        func=create_optimized_embedding_function(
            ...
            hf_tokenizer=hf_tokenizer,  # Pass HF tokenizer for HF binding
            hf_model=hf_model,  # Pass HF model for HF binding
        ),
    )
    ...
    # Initialize RAG-Anything with Native Configuration
    # Create RAGAnythingConfig with environment variables
    rag_config = RAGAnythingConfig(
        working_dir=args.working_dir,
        parse_method=os.getenv("PARSE_METHOD", "auto"),
        parser=os.getenv("PARSER", "mineru"),
        parser_output_dir=os.getenv("OUTPUT_DIR", str(args.input_dir)),
    ...
    def create_native_vision_model_func():
        """Create OpenAI-compatible vision model function for RAG-Anything"""
        def vision_model_func(prompt, system_prompt=None, history_messages=[], image_data=None, **kwargs):
            if image_data:
                ...
                # OpenAI standard format for multimodal content
                ...
                # Fall back to text-only LLM
                if args.llm_binding == "openai":
                    return openai_alike_model_complete(prompt, system_prompt, history_messages, **kwargs)
                elif args.llm_binding == "azure_openai":
                    return azure_openai_model_complete(prompt, system_prompt, history_messages, **kwargs)
                elif args.llm_binding == "aws_bedrock":
                    return bedrock_model_complete(prompt, system_prompt, history_messages, **kwargs)
                else:
                    return openai_alike_model_complete(prompt, system_prompt, history_messages, **kwargs)
        return vision_model_func
    
    # Create native RAG-Anything instance with full configuration
    rag_anything = RAGAnything(
        config=rag_config,
        llm_model_func=openai_complete if args.llm_binding == "openai" else (
            azure_openai_complete if args.llm_binding == "azure_openai" else (
                bedrock_complete if args.llm_binding == "aws_bedrock" else
                openai_complete
            )
        ),
        vision_model_func=create_native_vision_model_func(),
        embedding_func=embedding_func,
        lightrag_kwargs={
            "working_dir": args.working_dir,
            "workspace": args.workspace,
    ...
    logger.info("RAG-Anything initialized with native configuration and modal processors")
    ...
    app.include_router(create_query_routes(rag, api_key, args.top_k, rag_anything))
    app.include_router(create_graph_routes(rag, api_key, args.max_graph_nodes))
