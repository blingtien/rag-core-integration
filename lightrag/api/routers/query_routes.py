"""
This module contains all query-related routes for the LightRAG API with native RAG-Anything support.
"""
...
class QueryRequest(BaseModel):
    ...
    multimodal_content: Optional[List[Dict[str, Any]]] = Field(
        default=None,
        description="Multimodal content for enhanced queries. Each item should contain 'type' and appropriate data fields. "
                   "Supported types: 'image' (with 'image_data'), 'table' (with 'table_data', 'table_caption'), 'equation' (with 'latex', 'equation_caption').",
    )
...
class MultimodalQueryRequest(BaseModel):
    """Request model for multimodal queries with RAG-Anything native support"""
    ...
    mode: Literal["local", "global", "hybrid", "naive", "mix", "bypass"] = Field(
        default="hybrid",
        description="Query mode - hybrid recommended for multimodal content",
    )
    ...
    # Native RAG-Anything parameters
    context_window: Optional[int] = Field(
        default=None,
        ge=1,
        description="Context window size for multimodal processing",
    )
    
    max_context_tokens: Optional[int] = Field(
        default=None,
        ge=100,
        description="Maximum tokens for multimodal context",
    )
    
    enable_image_processing: Optional[bool] = Field(
        default=None,
        description="Enable image processing for multimodal content",
    )
    
    enable_table_processing: Optional[bool] = Field(
        default=None,
        description="Enable table processing for multimodal content",
    )
    
    enable_equation_processing: Optional[bool] = Field(
        default=None,
        description="Enable equation processing for multimodal content",
    )
...
def create_query_routes(rag, api_key: Optional[str] = None, top_k: int = 60, rag_anything=None):
    ...
    async def query_text(request: QueryRequest):
        ...
        **Usage Examples:**
        ...
        """
        try:
            # Use RAG-Anything's multimodal query if available and multimodal content is present
            if rag_anything is not None and request.multimodal_content is not None:
                response = await rag_anything.aquery_with_multimodal(
                    request.query,
                    multimodal_content=request.multimodal_content,
                    mode=request.mode,
                    # Pass native RAG-Anything parameters from environment or request
                ...
                # Convert RAG-Anything response format to match LightRAG format
                return QueryResponse(
                    response=response,
                    references=[],  # RAG-Anything handles references internally
                )

            # For all other cases (text-only queries), use LightRAG
            # Use LightRAG with unified approach
        ...
        try:
            ...
            if rag_anything is not None and request.multimodal_content is not None:
                response = await rag_anything.aquery_with_multimodal(
                    ...
                    multimodal_content=request.multimodal_content,
                    mode=request.mode,
                    # Pass native RAG-Anything parameters from environment
                    context_window=int(os.getenv("CONTEXT_WINDOW", "1")),
                    max_context_tokens=int(os.getenv("MAX_CONTEXT_TOKENS", "2000")),
                    enable_image_processing=os.getenv("ENABLE_IMAGE_PROCESSING", "true").lower() == "true",
                    enable_table_processing=os.getenv("ENABLE_TABLE_PROCESSING", "true").lower() == "true",
                ...
                # RAG-Anything responses are not streamable in current implementation
            ...
            # For all other cases (text-only queries), use LightRAG
            # Use LightRAG with streaming support
    ...
    async def query_multimodal(request: MultimodalQueryRequest):
        """
        Handle multimodal queries with RAG-Anything native support.

        This endpoint provides enhanced multimodal query capabilities using RAG-Anything
        with native parameter support for optimal multimodal content processing.

        Parameters:
            request (MultimodalQueryRequest): The request object containing query and multimodal content.

        Returns:
            QueryResponse: A response containing the multimodal query result.

        Raises:
            HTTPException: If RAG-Anything is not available or an error occurs during processing.
        """
        try:
            if rag_anything is None:
                raise HTTPException(
                    status_code=503,
                    detail="RAG-Anything is not available. Multimodal queries require RAG-Anything initialization."
                )

            # Use request-specific parameters if provided, otherwise fall back to environment defaults
            context_window = request.context_window or int(os.getenv("CONTEXT_WINDOW", "1"))
            max_context_tokens = request.max_context_tokens or int(os.getenv("MAX_CONTEXT_TOKENS", "2000"))
            enable_image = request.enable_image_processing
            if enable_image is None:
                enable_image = os.getenv("ENABLE_IMAGE_PROCESSING", "true").lower() == "true"
            enable_table = request.enable_table_processing
            if enable_table is None:
                enable_table = os.getenv("ENABLE_TABLE_PROCESSING", "true").lower() == "true"
            enable_equation = request.enable_equation_processing
            if enable_equation is None:
                enable_equation = os.getenv("ENABLE_EQUATION_PROCESSING", "true").lower() == "true"

            # Perform multimodal query with native RAG-Anything parameters
            response = await rag_anything.aquery_with_multimodal(
                request.query,
                multimodal_content=request.multimodal_content,
                mode=request.mode,
