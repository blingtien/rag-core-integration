...
class PGDocStatusStorage(DocStatusStorage):
    ...
    async def get_docs_paginated(
        ...
        # Build WHERE clause (exclude system-generated markdown files)
        where_clause = "WHERE workspace=$1 AND (file_path IS NULL OR NOT file_path LIKE '%.md')"
        params = {"workspace": self.workspace}
        param_count = 1
    ...
    async def get_all_status_counts(self) -> dict[str, int]:
        """Get counts of documents in each status for all documents (excludes system-generated markdown files)
        ...
        sql = """
            ...
            FROM LIGHTRAG_DOC_STATUS
            WHERE workspace=$1
            AND (file_path IS NULL OR NOT file_path LIKE '%.md')
            GROUP BY status
        """
        params = {"workspace": self.workspace}
        result = await self.db.query(sql, list(params.values()), True)
