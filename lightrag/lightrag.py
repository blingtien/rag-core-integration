...
class LightRAG:
    ...
    async def apipeline_enqueue_documents(
        ...
        if ids is not None:
            ...
            # Generate contents dict using file_path for stable IDs
            # This ensures the same file always gets the same doc_id, even if content extraction varies
            contents = {}
            for content, path in unique_content_with_paths.items():
                # Use file_path for ID generation to ensure consistency across retries
    ...
    async def apipeline_enqueue_error_documents(
        ...
        This function creates or updates error document entries in the doc_status storage for files
        that failed during the extraction process. If a document with the same file_path already exists,
        it updates that record instead of creating a new one to prevent duplicates.
        ...
        for error_file in error_files:
            ...
            existing_doc = await self.doc_status.get_doc_by_file_path(file_path)

            if existing_doc:
                # Use existing doc_id to update the record instead of creating a new one
                doc_id = existing_doc.get("id")
                created_at = existing_doc.get("created_at", current_time)
                logger.info(f"Updating existing document record for failed file: {file_path} (ID: {doc_id})")
            else:
                # Generate new doc_id using file_path only (same ID scheme as successful docs)
                # Use "doc-" prefix instead of "error-" so retries use the same ID
                doc_id = compute_mdhash_id(file_path, prefix="doc-")
                created_at = current_time
                logger.info(f"Creating new error document record for file: {file_path} (ID: {doc_id})")
            ...
            error_docs[doc_id] = {
                ...
                "created_at": created_at,  # Preserve original creation time if updating
