...
def create_openai_async_client(
    ...
    """
    if not api_key:
        # Try LLM_BINDING_API_KEY first, then OPENAI_API_KEY
        api_key = os.environ.get("LLM_BINDING_API_KEY") or os.environ.get("OPENAI_API_KEY")
        if not api_key:
            raise ValueError("API key not found. Set either LLM_BINDING_API_KEY or OPENAI_API_KEY environment variable.")
    ...
    if base_url is not None:
        ...
        # Try LLM_BINDING_HOST first, then OPENAI_BASE_URL, then OPENAI_API_BASE
        merged_configs["base_url"] = (
