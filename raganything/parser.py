...
class MineruParser(Parser):
    ...
    def _run_mineru_command(
        ...
        try:
            ...
            #         logging.warning("⚠️ MinerU处理失败，尝试备用PDF处理器...")
            #         MineruParser._run_fallback_pdf_processor(input_path, output_dir, method)
            #         logging.info("✅ 备用PDF处理器处理成功")
            #         return
            # except Exception as fallback_error:
            #     logging.error(f"❌ 备用PDF处理器也失败了: {fallback_error}")
            ...
            #     input_path_obj = Path(input_path)
            #     if input_path_obj.suffix.lower() == '.pdf':
            #         logging.warning("⚠️ MinerU未安装或配置不正确，使用备用PDF处理器...")
            #         MineruParser._run_fallback_pdf_processor(input_path, output_dir, method)
            #         logging.info("✅ 备用PDF处理器处理成功")
            #         return
            # except Exception as fallback_error:
    ...
    # @staticmethod
    # def _run_fallback_pdf_processor(
    #     input_path: Union[str, Path],
    #     output_dir: Union[str, Path],
    #     method: str = "auto"
    # ) -> None:
    #     """
    #     备用PDF处理器 - 当mineru失败时使用PyPDF2处理PDF
    #
    #     Args:
    #         input_path: PDF文件路径
    #         output_dir: 输出目录
    ...
    #     except ImportError:
    #         raise RuntimeError("PyPDF2 not installed. Please install it: pip install PyPDF2")
    #
    ...
    #
    #     # 提取PDF文本
    #     logging.info(f"使用备用处理器处理PDF文件: {input_path}")
    #
    #     try:
    ...
    #
    #     # 生成输出文件名（匹配MinerU的命名约定）
    #     base_name = input_path.stem
    #     markdown_file = output_dir / f"{base_name}.md"
    #     json_file = output_dir / f"{base_name}_content_list.json"  # 匹配MinerU格式
    #
    #     # 创建markdown内容
    #     markdown_content = f"# {base_name}\n\n{text}"
    #
    #     # 创建JSON结构（匹配MinerU的content_list格式）
    #     json_content = []
    #
    #     # 简化页面结构 - 转换为MinerU兼容的content_list格式
    #     pages = text.split("--- 第")
    #     for i, page_content in enumerate(pages[1:], 1):  # 跳过第一个空页面
    #         page_content = page_content.split(" 页 ---\n", 1)[-1] if " 页 ---\n" in page_content else page_content
    #         if page_content.strip():
    #             # 创建MinerU兼容的content block
    #             content_block = {
    #                 "type": "text",
    #                 "text": page_content.strip(),
    #                 "page_idx": i - 1,  # 0-based indexing
    #                 # 添加兼容性metadata（可选）
    #                 "_fallback_metadata": {
    #                     "parser": "fallback_pypdf2_processor",
    #                     "source_file": str(input_path),
    #                     "processing_method": "pypdf2_text_extraction",
    #                     "page_number": i
    #                 }
    #             }
    #             json_content.append(content_block)
    #
    #     # 写入文件
    ...
    #
    #     logging.info(f"备用处理器完成:")
    #     logging.info(f"- Markdown文件: {markdown_file}")
    #     logging.info(f"- JSON文件: {json_file}")
    #     logging.info(f"- 处理页数: {len(json_content)}")
    ...
    def parse_pdf(
        ...
        **kwargs,
        ...
        try:
            ...
            # Filter kwargs to only include supported parameters for _run_mineru_command
            supported_params = {
            ...
            # # 尝试使用备用PDF处理器
            # try:
            #     logging.warning("⚠️ MinerU处理失败，尝试使用备用PDF处理器...")
            #
            #     # 准备输出目录
            #     if output_dir:
            #         base_output_dir = Path(output_dir)
            ...
            #
            #     # 使用备用处理器
            #     self._run_fallback_pdf_processor(pdf_path, base_output_dir, method)
            #
            #     # 尝试读取输出文件
            #     content_list, _ = self._read_output_files(
            #         base_output_dir, name_without_suff, method=method
