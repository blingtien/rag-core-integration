...
// Batch upload files WITHOUT immediate processing (files marked as PENDING)
export const batchUploadWithoutProcessing = async (
  files: File[],
  onUploadProgress?: (percentCompleted: number) => void
): Promise<DocActionResponse> => {
  const formData = new FormData()
  ...
  const response = await axiosInstance.post('/documents/batch-upload', formData, {
    headers: {
      'Content-Type': 'multipart/form-data'
    },
...
// Trigger batch processing of all PENDING files
export const batchProcessPendingFiles = async (
  useMultimodal: boolean = true,
  maxConcurrent: number = 4
): Promise<DocActionResponse> => {
  const response = await axiosInstance.post('/documents/batch-process', null, {
    params: {
