...
import { RefreshCwIcon, ActivityIcon, ArrowUpIcon, ArrowDownIcon, RotateCcwIcon, CheckSquareIcon, XIcon, AlertTriangle, Info, PlayIcon } from 'lucide-react'
...
export default function DocumentManager() {
  ...
  const processPendingFiles = useCallback(async () => {
    try {
      ...
      console.log('Starting batch processing of pending files...');

      // Use max_concurrent=3 to match backend MAX_PARALLEL_INSERT setting
      const result = await batchProcessPendingFiles(true, 3);

      console.log('Batch processing result:', result);

      if (!isMountedRef.current) return;

      toast.success(result.message || result.status);
      ...
      if (isMountedRef.current) {
        toast.error(`Failed to process pending files: ${errorMessage(err)}`);
      }
    }
        ...
        <div className="flex justify-between items-center gap-2 mb-2">
          <div className="flex gap-2">
            ...
            <Button
              variant="outline"
              onClick={() => {
                console.log('Process Pending button clicked, statusCounts:', statusCounts);
                processPendingFiles();
              }}
              side="bottom"
              tooltip="Trigger batch processing for all PENDING files"
              size="sm"
              disabled={!statusCounts.pending || statusCounts.pending === 0}
            >
              <PlayIcon /> Process Pending ({statusCounts.pending || 0})
            </Button>
            <Button
              variant="outline"
